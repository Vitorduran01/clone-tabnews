...
export async function runTypeCheck(ts, baseDir, distDir, tsConfigPath, cacheDir, isAppDirEnabled) {
    ...
    if (firstError) {
        throw new CompileError(await getFormattedDiagnostic(ts, baseDir, distDir, firstError, isAppDirEnabled));
    }
    const warnings = await Promise.all(allDiagnostics.filter((d)=>d.category === DiagnosticCategory.Warning).map((d)=>getFormattedDiagnostic(ts, baseDir, distDir, d, isAppDirEnabled)));
    return {
        hasWarnings: true,
        warnings,
